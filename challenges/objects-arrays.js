...
let tyrannosaurus = dinasaur.filter(a => a.name === "tyrannosaurus");
console.log(tyrannosaurus.map(a => a.period));

// Create a new roar method for the tyrannosaurus.  When called, return "RAWERSRARARWERSARARARRRR!" Log the result.
let tyrannosaurus1 = dinasaur.filter(a => a.name === "tyrannosaurus");

console.log(tyrannosaurus1.map(a => a.roar()));
...
const universities = [];
graduates.forEach(a => universities.push(a.university));
...
const contactInfo = [];
graduates.forEach(a => contactInfo.push(a.first_name.concat(` ${a.email}`)));
console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called unisWithUni that contains them all. This will be an array of objects. Log the result. */
unisWithUni = graduates
  .filter(a => a.university.includes("Uni"))
  .map(a => a.university);

console.log("----------------University ONLY------------->");
...
zooAnimals.forEach(a =>
  displayNames.push(
    "Name: " + a.animal_name + ", Scientific: " + a.scientific_name
  )
); // animal_name and scientific_name
...
zooAnimals.map(a => lowCaseAnimalNames.push(a.animal_name.toLowerCase()));
console.log(lowCaseAnimalNames);

/* Request 3: .filter() 
The zoos are concerned about animals with a lower population count. 
Using filter, create a new array of objects called lowPopulationAnimals which contains only the 
animals with a population less than 5.
*/
const lowPopulationAnimals = [];

zooAnimals
  .filter(a => a.animal_name && a.population < 5)
  .map(b => lowPopulationAnimals.push(b.animal_name, b.population));
...
populationTotal = zooAnimals.reduce((acc, cur) => acc + cur.population, 0);
console.log("------------zooAnimals TOTAL Population ---------");
